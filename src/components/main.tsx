    ...
    <main id="main" className="pt-4 pb-4" ref={mainRef}>
      {
        showPages.length === 0 ? (
          process.env.NODE_ENV === 'development'
            ? <div className="btn btn-secondary main-input-upload" onClick={onClickImport}>导入</div>
            : <div className="alert alert-secondary text-center" role="alert">准备就绪<br/>请点击左侧上传按钮导入图片</div>
        ) : (
          showPages.map((row, i) => (
      ...
      <div className="row d-flex justify-content-end align-items-center mt-auto author-info">
        <div>{THIS_YEAR} zeplh@Github</div>
      </div>
    </main>
  )
}
